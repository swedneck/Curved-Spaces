	...
	Vector		*aNormalizedVector)	//	output, which may coincide with aRawVector
{
	...
	switch (aSpaceType)
	{
		...
		case SpaceFlat:
			...
			//	for flat space normalization, reorganize this function
	...
	Vector	*aVectorB)	//	assumed to be normalized to the SpaceType and not at infinity
{
	...
	{
		//	Do *not* call GeometryGamesErrorMessage() here.
		//	When we were supporting Win32, the Win32 UI would mess up the stack
		//	if we tried to put up a MessageBox while the OpenGL context is active.
		//	Now that we no longer support Win32 that problem is gone,
		//	but I haven't checked what other risks might be present
		//	if we tried to display an error message from here.
		return 0.0;
	}
}
